...
/**
 * 单个账户对同一合约品种单日进行认购和认沽期权的权利仓开仓的合计数量，包括买入开仓申报未成交部分（撤单需扣减）
 * 
 * @author yinhl
 */
public class Factor10002006 extends FactorBase {
	private static final long serialVersionUID = 4357613759099664671L;
	private Logger logger = LoggerFactory.getLogger(Factor10002002.class);
	private DataOptAsset dataOptAsset;

	private long sumOptPosiRltSingleDailyOpen = 0L;

	public Factor10002006() {
		super();
	}
		...
		try {
			...
			if (listOptAsset.size() > 0){
				for (DataOptAsset dataOptAssetResult : listOptAsset) {
					sumOptPosiRltSingleDailyOpen += dataOptAssetResult.getOptDailyOpenRlt();
				}
			}
		}catch (Exception e){
			throw new Exception("查表OPT_ASSET获取当日累计权利仓异常", e);
		}
	}

	@Override
	public Object getResult() throws Exception {
		return sumOptPosiRltSingleDailyOpen;
	}
}