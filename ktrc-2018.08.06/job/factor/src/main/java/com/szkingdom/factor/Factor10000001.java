...
/**
 * 单个账户持有同一合约品种的认购期权和认沽期权合约的权利仓合计不超过的数量
 * 
 * @author yinhl
 */
public class Factor10000001 extends FactorBase {
	private static final long serialVersionUID = 420709972123969187L;
	private Logger logger = LoggerFactory.getLogger(Factor10000001.class);
		...
		try{
			...
			if (custType == ConstantUtil.CUST_TYPE_NORMAL) {
				if (optCustType == ConstantUtil.OPT_CUST_TYPE_PERSON) {
					custLmtAttr = ConstantUtil.PLA_SINGLE_PERSON_LONG;
				}
				else if (optCustType == ConstantUtil.OPT_CUST_TYPE_ORG) {
					custLmtAttr = ConstantUtil.PLA_SINGLE_INST_LONG;
				}
			}
			else if (custType == ConstantUtil.CUST_TYPE_SELF_BUSI) {
				custLmtAttr = ConstantUtil.PLA_SINGLE_SECU_LONG;
			}
