 ...
 */
define("lib/kui/uploadify", [], function(require, exports, module) {
    $.fn.uploadify = function(options) {
        if (methods[options]) {
            return methods[options].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof options === "object" || !options) {
            return this.each(function() {
                methods.init.call(this, $.extend({}, $.fn.uploadify.defaults, $.fn.uploadify.parseOptions(this), options));
            });
        } else {
    ...
    $.fn.uploadify.parseOptions = function(target) {
        var jq = $(target);
        return $.extend({}, $.parser.parseOptions(target, [ "id", "butonText", "fileTypeDesc", "fileTypeExts", "fileSizeLimit", {
            multi: "boolean"
        } ]), {
            id: jq.attr("id")
        });
    ...
    $.fn.uploadify.defaults = {
        ...
        //读取文件
        readfile: "false",
        //附件保存文件夹名称 例如report,则附件上传后,放在/upload/report下,为空,则放在upload目录下
        uploadUrl: "../../kjdp_upload",
        //上传的路径
        buttonClass: "",
        //浏览文件按钮的样式
        buttonImage: "",
        ...
        fileSizeLimit: "10MB",
        multi: false,
        // 浏览文件窗口是否允许选择多个文件(多文件上传)
        IS_IMAGE_RESOURCE: "false",
        //是否上传到影像目录
        ...
        //仅供下载  无上传按钮
        autoCheck: true,
        //是否在初始化uploadify时检测flash版本
        showButton: true,
        //是否在初始化uploadify时显示上传按钮   ok
        showQueue: true,
        //是否在初始化uploadify时显示文件上传界面  ok
    ...
    var methods = {
        ...
        init: function(options) {
            var id = $(this).attr("id"), uploader = null;
            //上传的对象
            //上传的路径	
            options.uploadUrl = options.uploadUrl + (options.uploadUrl.indexOf("?") < 0 ? "?" : "&") + "DIR=" + options.dir + "&READFILE=" + options.readfile + "&IS_IMAGE_RESOURCE=" + options.IS_IMAGE_RESOURCE + "&LOCAL_RESOURCE=" + options.LOCAL_RESOURCE + "&UN_RENAME=" + options.UN_RENAME;
        ...
        disable: function(isDisabled) {
            ...
            uploader && uploader.disable.call(this, isDisabled);
        },
        // 上传队列中文件
        upload: function() {
            var uploader = $.data($(this)[0], "uploader"), args = arguments;
    ...
    Html5Upload.prototype.init = function(target, c) {
        ...
        var f = d.settings = $.extend({
            ...
            width: 100,
            fileType: {
                image: [ "jpg", "png", "bmp", "gif", "jpeg", "tif" ]
            }
        }, c);
        ...
        if (isNaN(f.fileSizeLimit)) {
            ...
            if (f.fileSizeLimit.indexOf("KB") > -1) {
                f.fileSizeLimit = e * 1e3;
            } else {
                if (f.fileSizeLimit.indexOf("MB") > -1) {
                    f.fileSizeLimit = e * 1e6;
                } else {
                    if (f.fileSizeLimit.indexOf("GB") > -1) {
                        f.fileSizeLimit = e * 1e9;
                    }
                }
            }
        } else {
            ...
            fileInput.bind("change", function() {
                ...
                if (d.queue.count + l > f.fileNumLimit && f.fileNumLimit !== 0) {
                    if ($.inArray("onError", f.overrideEvents) < 0) {
                        alert("选定的文件数量超过设置的上传文件数 (" + f.fileNumLimit + ")！");
                    }
                    ...
                    var checkfileType = function(_file) {
                        var result = true;
                        if (f.fileTypeExts) {
                            var _fileType = _file.name.split(".")[1], types = f.fileTypeExts.split(";");
                            result = false;
        ...
        d.drop = function(m) {
            ...
            d.queue.selected = j;
            if (d.queue.count + j > f.fileNumLimit && f.fileNumLimit !== 0) {
                if ($.inArray("onError", f.overrideEvents) < 0) {
                    alert("选定的文件数量超过设置的上传文件数  (" + f.fileNumLimit + ")！");
                }
                if (typeof f.onError === "function") {
        ...
        if (f.itemTemplate == false) {
            d.queueItem = $('<div class="uploadify-queue-item"><div class="cancel"><a class="close" href="#"></a></div><div>' + '<span class="filename"></span><span class="fileinfo"></span></div>' + '<div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></div>');
        } else {
        ...
        d.addQueueItem = function(k) {
            ...
            if (f.fileTypeExts) {
                var name = k.name.split("."), _fileType = name[name.length - 1], types = f.fileTypeExts.split(";"), result = false;
                for (var i = 0, typesLen = types.length; i < typesLen; i++) {
                    if (types[i].split(".")[1] == _fileType) {
                        result = true;
                        break;
            ...
            if (d.queue.count + 1 > f.fileNumLimit && f.fileNumLimit !== 0) {
                if ($.inArray("onError", f.overrideEvents) < 0) {
        ...
        d.removeQueueItem = function(m, l, k) {
            ...
            if (m.queueItem) {
                ...
                if (m.queueItem.hasClass("upload-error")) {
                    return;
                }
                // 灵活设置跳转路径
        ...
        d.error = function(l, j, k) {
            ...
            if ($.inArray("onError", f.overrideEvents) < 0) {
                ...
                alert(errorMsg);
                j.queueItem.delay(1e3 + 100 * delay).fadeOut(500, function() {
                    $(this).remove();
                });
            }
            if (typeof f.onError === "function") {
        ...
        d.uploadComplete = function(l, j, k) {
            ...
            var uploadInfo = eval("(" + data + ")"), fileData = uploadInfo.data;
            if ($.inArray("onUploadSuccess", f.overrideEvents) < 0) {
                j.queueItem.find(".uploadify-progress-bar").css("width", "100%");
                j.queueItem.find(".progress").slideUp(250);
                if (uploadInfo.flag === false) {
                    j.queueItem.addClass("upload-error complete").find(".fileinfo").html(" - 失败:[" + (uploadInfo.msg || "") + "]");
                } else {
                    j.queueItem.addClass("complete").find(".fileinfo").html(" - 成功");
                }
            }
            if (uploadInfo.flag === false) {
                return;
            ...
            if (typeof f.onUploadSuccess === "function") {
                f.onUploadSuccess.call(this, fileData.length ? fileData[0] : {}, fileData, uploadInfo);
            }
            for (var i = 0; i < fileData.length; i++) {
                ...
                fileWrapper.append(cancel).append(fileSpan).addClass("complete").addClass("upload-success");
            ...
            d.queueEl.find("span a").click(function() {
                window.open(resolve("kjdp_download?DIR=" + f.dir + "&FILENAME=" + $(this).attr("FILENAME") + "&FILECON=" + $(this).attr("FILECON")));
            });
            var $this = this;
            d.queueEl.find("div.cancel:last a").click(function(e) {
                var FILECON = $(this).attr("FILECON"), FILENAME = $(this).attr("FILENAME"), ATT_ID = $(this).attr("ATT_ID");
                $.get(resolve("kjdp_delete?DIR=" + f.dir + "&FILECON=" + FILECON), {}, function(data, textStatus) {
                    ...
                    if (hiddenIptVal && hiddenIptVal != "") {
                        ...
                        var newCons = $.grep(cons, function(n) {
                            return n != ATT_ID;
                        });
        ...
        if (window.File && window.FileList && window.Blob && (window.FileReader || window.FormData)) {
            ...
            d.button = $('<div id="' + f.id + '">' + textStr + "</div>");
            if (f.showButton) {
            ...
            if (!f.queueID) {
                ...
                d.queueEl = $('<div id="' + f.queueID + '" class="uploadify-queue" />');
                box.after(d.queueEl);
            } else {
                d.queueEl = $("#" + f.queueID);
            }
            if (f.dnd) {
                var h = f.dropTarget ? $(f.dropTarget) : d.queueEl.get(0);
    ...
    Html5Upload.prototype.cancel = function(target, d, c) {
        ...
        if (typeof d === "string") {
            if (!isNaN(d)) {
                fileID = "uploadify-" + target.attr("id") + "-file-" + d;
            }
            d = $("#" + fileID).data("file");
        }
        d.skip = true;
    ...
    Html5Upload.prototype.upload = function(target, c, d) {
        ...
        if (c) {
            ...
            if (state.uploads.count + state.uploads.current < settings.uploadLimit || settings.uploadLimit == 0) {
                if (!d) {
                ...
                $("#" + settings.queueID).find(".uploadify-queue-item").not(".error, .complete").each(function() {
                    ...
                    if (state.uploads.current >= settings.simUploadLimit && settings.simUploadLimit !== 0 || state.uploads.current >= settings.uploadLimit && settings.uploadLimit !== 0 || state.uploads.count >= settings.uploadLimit && settings.uploadLimit !== 0) {
                        return false;
                    }
                ...
                if (state.uploads.current == 0) {
                    if ($.inArray("onError", settings.overrideEvents) < 0) {
                        if (state.filesToUpload() > 0 && settings.uploadLimit != 0) {
                            alert("The maximum upload limit has been reached.");
                        }
                    }
                    if (typeof settings.onError === "function") {
                        settings.onError.call($this, "UPLOAD_LIMIT_EXCEEDED", state.filesToUpload());
                    }
                }
            }
        }
    ...
    FlashUpload.prototype.init = function(target, options, swfUploadOptions) {
        ...
        if ($.browser.msie || $.browser.webkit) $(window).unload(function() {
        ...
        return $this.each(function() {
            ...
            var settings = $.extend({
                ...
                swf: resolve("lib/kui/swf/uploadify.swf#"),
            ...
            if (!settings.queueID) {
                var $queue = settings.showQueue ? $('<div style="width:auto;"/>').attr("id", settings.id + "-queue").attr("class", "uploadify-queue") : $();
                $wrapper.after($queue);
        ...
        return {
            ...
            onSelect: function(file) {
                ...
                if (settings.itemTemplate == false) {
                    settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">    					<div class="cancel">    						<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')"></a>    					</div>    					<span class="FILENAME">${FILENAME}</span><span class="data"></span>    					<div class="uploadify-progress">    						<div class="uploadify-progress-bar"><!--Progress Bar--></div>    					</div>    				</div>';
                }
                // Run the default event handler
            ...
            onUploadSuccess: function(file, data, response) {
                if (!$.trim(data)) {
                    alert("未获取到服务器返回的数据！", alert.ERROR);
                    return;
                }
                // Load the swfupload settings
                var settings = this.settings;
                ...
                var uploadInfo = eval("(" + data + ")"), fileData = uploadInfo.data;
                if (uploadInfo.flag === false) {
                    $("#" + file.id).find(".data").html(" - 失败:[" + (uploadInfo.msg || "") + "]");
                    return;
                }
                if (!settings.showResult) {
                    ...
                    $.isFunction(settings.onUploadSuccess) && settings.onUploadSuccess.call(this, file, data, response);
                    return;
                }
                for (var i = 0; i < fileData.length; i++) {
                    var file = fileData[i];
                ...
                this.queue.find("span:last a").click(function() {
                    window.open(resolve("kjdp_download?DIR=" + settings.dir + "&FILENAME=" + $(this).attr("FILENAME") + "&FILECON=" + $(this).attr("FILECON")));
                });
                var $this = this;
                this.queue.find("div.cancel:last a").click(function(e) {
                    var FILECON = $(this).attr("FILECON"), FILENAME = $(this).attr("FILENAME"), ATT_ID = $(this).attr("ATT_ID");
                    $.get(resolve("kjdp_delete?DIR=" + settings.dir + "&FILECON=" + FILECON), {}, function(data, textStatus) {
                        var hiddenIpt = $(target).find("input[name][name=" + targetName + "]");
                        var hiddenIptVal = hiddenIpt.val();
                        if (hiddenIptVal != "") {
                            var cons = hiddenIptVal.split(",");
                    ...
                    e.preventDefault(); else //IE中阻止函数器默认动作的方式
                    window.event.returnValue = false;
    ...
    FlashUpload.prototype.cancel = function(fileID, supressEvent) {
        ...
        this.each(function() {
            ...
            var $this = $(this), swfuploadify = $.data($(this)[0], "uploadify"), settings = swfuploadify.settings, delay = -1;
            if (args[0]) {
                // Clear the queue
                if (args[0] == "*") {
    ...
    FlashUpload.prototype.destroy = function(target) {
        target.each(function() {
            // Create a reference to the jQuery DOM object
            var $this = $(this), swfuploadify = $.data($(this)[0], "uploadify"), settings = swfuploadify.settings;
    ...
    FlashUpload.prototype.settings = function(target, name, value, resetObjects) {
        ...
        target.each(function() {
            // Create a reference to the jQuery DOM object
            var $this = $(this), swfuploadify = $.data($(this)[0], "uploadify");
    ...
    FlashUpload.prototype.showFiles = function(target) {
        ...
        this.each(function() {
            ...
            var uploadFileInfo = args[0];
            if (uploadFileInfo && uploadFileInfo.flag === true) {
                var fileData = uploadFileInfo.data;
                for (var i = 0; i < fileData.length; i++) {
                    var file = fileData[i];
                    var fileLength = file.FILENAME.length;
                    if (fileLength > 24) {
