 ...
 */
define("lib/kui/tabs", [], function(require, exports, module) {
    ...
    $.fn.tabs.parseOptions = function(target) {
        return $.extend({}, $.parser.parseOptions(target, [ "width", "height", "tools", "toolPosition", "tabPosition", {
            fit: "boolean",
            border: "boolean",
            plain: "boolean",
            headerWidth: "number"
        ...
        if (tabsWidth > cWidth) {
            ...
            if (opts.toolPosition == "left") {
                ...
                wrap.css({
                    ...
                    width: cWidth - sLeft.outerWidth() - sRight.outerWidth() - 25
                });
            } else {
                tool.css({
                    left: "",
    ...
    function wrapTabs(container) {
        ...
        $('<div class="tabs-header">' + '<div class="tabs-scroller-left"></div>' + '<div class="tabs-scroller-right"></div>' + '<div class="tabs-wrap">' + '<ul class="tabs clearfix"></ul>' + "</div>" + "</div>").prependTo(container);
        cc.children("div.tabs-panels").children("div").each(function(i) {
            var panelOpts = $.extend({}, $.parser.parseOptions(this), {
                selected: $(this).attr("selected") ? true : undefined,
                fullScreenAble: opts.fullScreenAble || false
            });
            var pp = $(this);
            tabs.push(pp);
            createTab(container, pp, panelOpts);
        });
    ...
    function createTab(container, pp, options) {
        ...
        opts.tab = $("<li " + idString + "></li>").appendTo(tabs);
        // set the tab object in panel options
        opts.tab.append('<a href="javascript:void(0)" class="tabs-inner">' + '<span class="tabs-title"></span>' + (opts.iconCls ? '<span class="tabs-icon"></span>' : "") + "</a>");
    ...
    function addPageTab(container, options) {
        if (!tabIdExists(container, options.id)) {
            addTab(container, {
                id: options.id,
                title: options.title,
                content: '<iframe id="' + options.id + '" nodeid="' + options.id + '" scrolling="auto" frameborder="0" allowtransparency="true"  style="width:100%;height:100%;overflow:auto;"></iframe>',
    ...
    function updateTab(container, param) {
        var tabOpts = $.data(container, "tabs").options;
        ...
        if (s_title.width() >= tabOpts.maxTitleWidth) {
            s_title.css({
                display: "block",
                width: tabOpts.maxTitleWidth - 20 + "px",
    ...
    function selectTab(container, which, isId) {
        ...
        var panel;
        if (isId) panel = getTabById(container, which); else panel = getTab(container, which);
        // get the panel to be activated
        if (!panel) return;
        if (panel.panel("options").tab.hasClass("tabs-selected")) {
            opts.onSelect.call(container, panel.panel("options").title || "", getTabIndex(container, panel));
            return false;
        }
    ...
    function destroy(target) {
        var index = 0, tabPanel;
        while ((tabPanel = getTab(target, index++)) && tabPanel.length) {
