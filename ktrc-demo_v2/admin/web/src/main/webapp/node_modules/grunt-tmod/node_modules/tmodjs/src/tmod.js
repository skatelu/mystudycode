...
var Tmod = function (base, options) {
    ...
    // 输出路径
    this.output = options.output && path.resolve(this.base, options.output);


    // 运行时输出路径
    this.runtime = options.output ? path.resolve(this.output, options.runtime) : path.resolve(this.base, options.dest);
    ...
    // 输出运行时 这个时机需要优化     @lipr 添加参数未配置时不输出
    options.dest && this._buildRuntime();
};
...
Tmod.prototype = {
    ...
    _compile: function (file) {
        ...
        // 目标路径
        var target = file.replace(path.extname(file), '.js');
        this.output && (target = target.replace(this.base, this.output));
