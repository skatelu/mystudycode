...
public class DruidDataSourceFactory implements DataSourceFactory {
	...
	public DataSource getDataSource() {
		...
		if (props == null || props.getProperty("driverClassName") == null) {
			try {
				input = ConfigFileUtil.readPropFileOutJar("/usr/local/apache-storm-1.1.0/config/druid.properties");
			} catch (Exception e) {
				logger.error("未能成功外部读取/config/druid.properties，下面尝试读取内部读取");
			}
			try {
				if (input == null) {
					input = ConfigFileUtil.readPropFileInJar("druid.properties");
				}
