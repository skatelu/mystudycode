...
public class Sender implements Runnable {
    ...
    public void run() {
        ...
        //如果不是强制关闭，且消息累加器accumulator尚有消息未发送，或者客户端client尚有正在处理（in-flight）
        ...
        //如果是强制关闭；调用消息累加器accumulator的abortIncompleteBatches（）方法，放弃未处理完的请求
        if (forceClose) {
    ...
    private long sendProducerData(long now) {
        ...
        // 如果到现在还有任何分区不知道自己的领导者，强制刷新matadata元数据
        ...
        //Integer - broker id   batches - 要发送的batch
        /**
         *  drain() 函数作用：对给定的节点，从缓冲器中抽取节点下作为leader的topic partition 的Batch集合
         *      遍历给定节点
         *      获取该节点下每个作为leader的topic partition（不包括处于muted中的）
         *      获取该缓冲器中topic partition的Batch队列
         *      查看第一个Batch，若不为空且不处于重发策略，且不超过request最大大小，则抽出这个Batch准备发送。
         *     （特例：若只有一个Batch且超过最大request，也会抽取出去）
         */
        Map<Integer, List<ProducerBatch>> batches = this.accumulator.drain(cluster, result.readyNodes, this.maxRequestSize, now);
        addToInflightBatches(batches);
        //把要发送的batches 加入到 RecordAccumulator 的 muted 中
