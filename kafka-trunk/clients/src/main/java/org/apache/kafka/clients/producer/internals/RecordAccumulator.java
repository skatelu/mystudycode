...
public final class RecordAccumulator {
     ...
     * </ol>
     */
    //返回处于就绪状态的节点列表，以及未知的leader的topic
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        //遍历所有的TopicPartition队列
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            TopicPartition part = entry.getKey();
            Deque<ProducerBatch> deque = entry.getValue();

            //获取该TopicPartition 的leader  broker
            Node leader = cluster.leaderFor(part);
            synchronized (deque) {
                //若此TopicPartition中的leader未知，且队列中存在待发送的ProducerBatch，后续sender会据此更新Metadata
                if (leader == null && !deque.isEmpty()) {
                    // This is a partition for which leader is not known, but messages are available to send.
                    // Note that entries are currently not removed from batches when deque is empty.
                    unknownLeaderTopics.add(part.topic());
                    // 注意：这里排除了已经放在 muted 中的 TopicPartition, 是为了保证消息顺序（即：MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION = 1），
                    // 也就是若TopicPartition存在正处于发送中的request（未确认），则不属于就绪状态
                    ...
                    if (batch != null) {
                        ...
                        //Batch是否处于重发策略中，即还是在等待下次重发
                        boolean backingOff = batch.attempts() > 0 && waitedTimeMs < retryBackoffMs;

                        //需要等待的时间：
                        //      若处于重发策略，等于 配置中的 retry.backoff.ms 时间，默认100ms
                        //      否则，等于 配置中的linger.ms 时间 默认 0ms
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        //Batch  是否已满
                        boolean full = deque.size() > 1 || batch.isFull();
                        //是否过时，也就是已经等待时间超过了需要等待时间
    ...
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        do {
            ...
            synchronized (deque) {
                ...
                if (backoff)
                    continue;
                //注意：如果加上这个Batch的超过了request最大大小限制，但是该节点下还没有就绪数据
